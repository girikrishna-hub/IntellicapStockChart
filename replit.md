...
- **Improved Earnings Date Accuracy:** Enhanced earnings data processing to ensure recent earnings dates are properly displayed, with robust fallback logic that prioritizes current year data while ensuring no earnings are missed due to overly restrictive date filtering. Added detection for potentially outdated earnings data and user warnings when data sources may be missing recent announcements. Fixed duplicate earnings dates issue where next earnings showed same date as last earnings by adding proper future date validation and estimation fallback logic
