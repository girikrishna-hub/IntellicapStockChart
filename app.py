        ...
        try:
            ...
            try:
                ...
                if earnings_dates is None or earnings_dates.empty:
                    raise ValueError("No earnings dates available")
                
                print(f"Found {len(earnings_dates)} earnings records from earnings_dates")
                
                # Create quarters lookup
                quarters_data = []
                unique_quarters = set()
                ...
                for date, row in earnings_dates.iterrows():
                    ...
                    if quarter_key not in unique_quarters:
                        unique_quarters.add(quarter_key)
                ...
                for quarter_info in quarters_data:
                    earnings_date = quarter_info['Date']
                    print(f"Processing earnings date: {earnings_date}")
                    
                    try:
                        # Get pre-earnings date (day before earnings)
                        ...
                        if not historical_data.empty:
                            ...
                            pre_earnings_data = historical_data[historical_data.index <= pre_earnings_date]
                            if not pre_earnings_data.empty:
                                pre_earnings_close = pre_earnings_data['Close'].iloc[-1]
                                
                                # Find post-earnings data
                                post_earnings_data = historical_data[historical_data.index > earnings_date]
                                if not post_earnings_data.empty:
                                    # Next day performance
                                    next_day_close = post_earnings_data['Close'].iloc[0]
                                    overnight_change = ((next_day_close - pre_earnings_close) / pre_earnings_close) * 100
                                    
                                    # Week performance (5 trading days later)
                                    if len(post_earnings_data) >= 5:
                                        week_close = post_earnings_data['Close'].iloc[4]
                                        week_change = ((week_close - pre_earnings_close) / pre_earnings_close) * 100
                                    else:
                                        week_close = post_earnings_data['Close'].iloc[-1]
                                        week_change = ((week_close - pre_earnings_close) / pre_earnings_close) * 100
                                    ...
                                    analysis_results.append({
                                        ...
                                        'Overnight_%': overnight_change,
                                        'NextDay_%': overnight_change,  # Same as overnight for this analysis
                                        'Week_%': week_change
                                    })
                                    
                                    print(f"✅ Analysis successful: {overnight_change:.2f}% overnight, {overnight_change:.2f}% next day, {week_change:.2f}% week")
                                else:
                                    print(f"No post-earnings data found for {earnings_date}")
                            else:
                                print(f"No pre-earnings data found for {earnings_date}")
                        else:
                            print(f"No historical data available for analysis")
                            
                    except Exception as e:
                        print(f"Error analyzing {earnings_date}: {str(e)}")
                        continue
                
                # Create DataFrame
                if analysis_results:
                    earnings_analysis = pd.DataFrame(analysis_results)
                    print(f"Created analysis DataFrame with {len(earnings_analysis)} rows")
                else:
                    earnings_analysis = pd.DataFrame()
                    print("No earnings analysis data could be generated")
